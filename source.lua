...
function lib:init(ti, dosplash, visiblekey, deleteprevious)
	...
	Notificationshadow.Parent = notif;
	Notificationshadow.AnchorPoint = Vector2.new(0.5, 0.5);
	Notificationshadow.BackgroundTransparency = 1;
	Notificationshadow.Position = UDim2.new(0.5, 0, 0.5, 0);
	Notificationshadow.Size = UDim2.new(1.20000005, 0, 1.20000005, 0);
	Notificationshadow.Image = "rbxassetid://313486536";
	...
	notifdarkness.BackgroundTransparency = 0.6;
	...
	notif2darkness.BackgroundTransparency = 0.6;
	...
	function window:GreenButton(callback)
		if _G.gbutton_123123 then
			_G.gbutton_123123:Disconnect();
		end;
	...
	function window:TempNotify(text1, text2, icon)
		...
		tempnotif.BackgroundTransparency = 0.15;
	...
	function window:Notify(txt1, txt2, b1, icohn, callback)
		if notif.Visible == true or notif2.Visible == true then
		...
		if callback then
			PrimaryCallback = notifbutton1.MouseButton1Click:Connect(function()
				PrimaryCallback:Disconnect();
				callback();
				notif.Visible = false;
			end);
		end;
	...
	function window:Notify2(txt1, txt2, b1, b2, icohn, callback, callback2)
		...
		if callback and callback2 then
			PrimaryCallback = notif2button1.MouseButton1Click:Connect(function()
				PrimaryCallback:Disconnect();
				SecondaryCallback:Disconnect();
				callback();
				notif2.Visible = false;
			end);
			SecondaryCallback = notif2button2.MouseButton1Click:Connect(function()
				PrimaryCallback:Disconnect();
				SecondaryCallback:Disconnect();
				callback2();
				notif2.Visible = false;
	...
	function window:Section(name)
		...
		table.insert(ContentAreas, workareamain);
		local sec = {};
		function sec:Select()
			...
			for b, v in next, ContentAreas do
				v.Visible = false;
			end;
